...
export function TenantsTable({ tenants }: { tenants: Tenant[] }) {
  ...
  const filteredTenants = useMemo(() => {
    return tenants.filter((t) => {
      if (filters.name && !t.full_name?.toLowerCase().includes(filters.name.toLowerCase())) return false
      if (filters.document && !t.document_number?.toLowerCase().includes(filters.document.toLowerCase())) return false
    ...
    return (
      <Card className="p-12 text-center">
        ...
        <Button asChild className="mt-4">
          <Link href="/arrendatarios/nuevo">Agregar Primer Inquilino</Link>
        </Button>
      </Card>
    )
  }
  ...
  return (
    <Card className="overflow-hidden">
      <div className="p-4 border-b bg-muted/30 flex flex-wrap gap-3 items-center">
        ...
        <span className="text-sm text-muted-foreground ml-auto">{filteredTenants.length} inquilinos</span>
      </div>
                ...
                <TableCell className="text-right">
                  <div className="flex justify-end gap-1">
                    <Button variant="ghost" size="icon" asChild>
                      <Link href={`/arrendatarios/${tenant.id}`}>
