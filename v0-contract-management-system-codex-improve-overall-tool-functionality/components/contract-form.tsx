...
export function ContractForm({
  ...
  const tenantOptions = tenants.map((t) => ({
    value: t.id,
    label: t.full_name,
    description: t.email || t.document_number || "",
  }))

  const landlordOptions = landlords.map((l) => ({
    value: l.id,
    label: l.full_name,
    description: l.email || l.document_number || "",
  }))

  const guarantorOptions = guarantors.map((g) => ({
    value: g.id,
    label: g.full_name,
    description: g.email || g.document_number || "",
  }))
    ...
    while (currentDate <= end) {
      ...
      if (increaseType && increaseFrequencyMonths > 0 && paymentNumber > 1) {
        if ((paymentNumber - 1) % increaseFrequencyMonths === 0) {
          ...
          if (increaseType === "porcentaje_fijo" && increasePercentage > 0) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    const contractData = {
      ...
      deposit_amount: monthlyRent * depositMonths,
      ...
      late_payment_fee_percentage: latePaymentFeePercentage,
