...
import { Building2, FileText, Users, TrendingUp, AlertCircle, Calendar, AlertTriangle, Info } from "lucide-react"
...
export default async function DashboardPage() {
  ...
  const next3Months = new Date(currentYear, currentMonth + 3, today.getDate())
  ...
  const [
    ...
    supabase
      .from("payments")
      .select(`*, contracts (contract_number, tenants (full_name))`)
      .eq("status", "atrasado")
  ...
  const contractsWithIncreasesThisMonth = (allActiveContracts || []).filter((contract: ContractWithRelations) => {
    ...
    if (appliedContractIds.has(contract.id)) return false
    ...
    while (
      increaseDate <= today ||
      (increaseDate.getMonth() === currentMonth && increaseDate.getFullYear() === currentYear)
    ) {
      if (increaseDate.getMonth() === currentMonth && increaseDate.getFullYear() === currentYear) {
        return true
      }
      increaseDate.setMonth(increaseDate.getMonth() + frequencyMonths)
    }
  ...
  return (
    <div className="space-y-6 max-w-[1400px] mx-auto">
      <div>
        ...
        <p className="text-muted-foreground">Resumen de gestión de contratos</p>
      </div>
      ...
      <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-4">
        <Card className="border-l-4 border-l-blue-500">
          ...
          <CardContent>
            <div className="text-2xl font-bold text-blue-600">{activeContractsCount || 0}</div>
            <p className="text-xs text-muted-foreground">{contractsNearExpiry?.length || 0} por vencer próximamente</p>
          </CardContent>
        </Card>
        ...
        <Card className="border-l-4 border-l-emerald-500">
          ...
          <CardContent>
            ...
            <p className="text-xs text-muted-foreground">{occupancyRate}% de ocupación</p>
          </CardContent>
        </Card>
      ...
      <div className="grid gap-6 lg:grid-cols-2">
        ...
        <Card className="border-t-4 border-t-amber-500">
          <CardHeader>
            <div className="flex items-center justify-between">
              <div>
                ...
                <CardDescription>Contratos con aumentos este mes (sin aplicar)</CardDescription>
              </div>
              <Button variant="ghost" size="sm" asChild>
          ...
          <CardContent>
            {contractsWithIncreasesThisMonth && contractsWithIncreasesThisMonth.length > 0 ? (
              <div className="space-y-3 max-h-[400px] overflow-y-auto">
                    ...
                    >
                      ...
                      <div className="text-right">
                        <p className="text-sm font-medium">
                          {formatCurrency(contract.monthly_rent, contract.currency as CurrencyType)}
                        </p>
        ...
        <Card className="border-t-4 border-t-red-500">
          <CardHeader>
            <div className="flex items-center justify-between">
              <div>
                ...
                <CardDescription>Atrasos en pagos</CardDescription>
              </div>
              <Button variant="ghost" size="sm" asChild>
          ...
          <CardContent>
            {overduePayments && overduePayments.length > 0 ? (
              <div className="space-y-3">
                {overduePayments.map((payment: PaymentWithRelations) => {
                  ...
                  return (
                    ...
                    >
                      ...
                      <div className="text-right">
                        ...
                        <p className="text-xs text-red-600">{daysOverdue} días de atraso</p>
                      </div>
                    </div>
                  )
                })}
              ...
              <p className="text-sm text-muted-foreground text-center py-8">No hay pagos atrasados</p>
            )}
          </CardContent>
        </Card>
      </div>
